...
def test(dataset_frame1, dataset_frame2, dataset_frame3):
  ...
  with tf.Graph().as_default():
    ...
    for id_img in range(0, data_size):
      ...
      batch_data_frame1 = [dataset_frame1.process_func(
          ll) for ll in data_list_frame1[0:63]]
      batch_data_frame2 = [dataset_frame2.process_func(
          ll) for ll in data_list_frame2[0:63]]
      batch_data_frame3 = [dataset_frame3.process_func(
          ll) for ll in data_list_frame3[0:63]]
      ...
      i += 1
      PSNR += 10*np.log10(255.0*255.0 /
                          np.sum(np.square(prediction_np-target_np)))
